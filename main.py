...
def render_timer(deadline_str):
    ...
    if deadline_dt:
        ...
        if secs > 0:
            ...
            st.error("⚠️ OVERDUE")
            st.metric("Time Overdue", f"{abs(secs)//60}m {abs(secs)%60}s") [site: 22]
...
if st.session_state.role is None:
    st.title("🚩 ZSM Task Control Center") [site: 27-28]
    c1, c2 = st.columns(2)
    with c1:
...
if st.session_state.role == "Admin":
    ...
    if menu == "Dashboard":
        st.title("👨‍💼 Task Assignment") [site: 32]
        u_df = get_users()
        ...
        df = get_tasks()
        st.dataframe(df, use_container_width=True) [site: 44]
        ...
        c_df = get_companies()
        st.dataframe(c_df, use_container_width=True) [site: 85]

    # ==========================================
    # 4. EMPLOYEE VIEW
    # ==========================================
    elif st.session_state.role == "Employee":
        st.title(f"👷 {st.session_state.user}'s Workspace") [site: 92]
        df = get_tasks()
            ...
            with st.container(border=True):
                ...
                if row["Status"] == "Pending" and st.button("▶️ START", key=f"s_{idx}"):
                    ...
                    if st.button("✅ FINISH", key=f"f_{idx}"):
                        ...
                        df.at[idx, "Submit_Time"] = get_now_ist().strftime("%Y-%m-%d %I:%M:%S %p")
                        save_tasks(df); st.rerun()
